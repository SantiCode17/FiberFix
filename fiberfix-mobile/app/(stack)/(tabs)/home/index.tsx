...
type RootStackParamList = {
  historial: { ticketId: string };
  // Otras rutas pueden añadirse aquí si es necesario
};

export default function TicketScreen() {
  const { userId } = useUser();
  //const userId = 'TEC001'; // ID de técnico fijo para pruebas
  ...
  const submitIncident = () => {
    ...
    if (!incidentNote.trim()) {
      CustomAlert({
        title: 'Faltan datos',
        message: 'Selecciona un motivo o escribe una descripción.',
      ...
      cliente.on('data', (data) => {
        ...
        if (response.startsWith('START_OK_EXISTENTE')) {
          setAlertConfig({
            ...
            message: 'El ticket ya existe. ¿Qué deseas hacer?',
            buttons: [
              { text: 'Cancelar', onPress: () => setTicketNumber(''), style: 'cancel' },
              { text: 'Ver detalles', onPress: () => router.push(`/(stack)/(tabs)/historial/explore?ticketId=${ticketNumber}`) },
            ],
          });
        ...
        } else if (response.startsWith('INCIDENT_OK')) {
          setStatusMessage({ text: 'Incidencia registrada correctamente', type: 'success' });
          setShowIncidentModal(false);
          setIsWorking(false);
          setTicketNumber(''); // Limpiar el ticket después de registrar la incidencia
        } else {
              ...
              >
                <View className="bg-white rounded-t-[40px] h-[92%] shadow-2xl overflow-hidden">
                  ...
                  >
                    ...
                    <Text className="text-fiber-blue font-black text-sm uppercase mb-3">1. Descripción Detallada</Text>
                    <TextInput
                    ...
                    <Text className="text-fiber-blue font-black text-sm uppercase mb-3">2. Evidencia Visual</Text>
                    <TouchableOpacity

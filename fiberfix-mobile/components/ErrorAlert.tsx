...
export const ErrorAlert: React.FC<ErrorAlertProps> = ({ error, onDismiss }) => {
  ...
  const getErrorConfig = (type: ErrorType) => {
    const configs: Record<
      ErrorType,
      { icon: any; color: string; bg: string; textColor: string }
    > = {
      error: {
      ...
      <View className="flex-1 bg-black/50 justify-center items-center p-6">
        ...
        >
          ...
          <View className="gap-3">
            ...
            >
              <Text className="text-gray-700 font-black uppercase text-sm tracking-widest">Cancelar</Text>
            </TouchableOpacity>
          </View>
          {/* Detalles técnicos (small text) */}
          <Text className="text-xs text-gray-400 mt-6 text-center">Si el problema persiste, contacta con soporte</Text>
        </Animated.View>
      </View>
    </Modal>
  );
...
export const useErrorHandler = () => {
  ...
  const showServerError = (
    message: string = 'No se puede conectar con el servidor. Verifica la IP y puerto.',
    action?: any
  ) => {

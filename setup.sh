...
docker run -d --name add-dbms --network edu-shared \
  -e MYSQL_ROOT_PASSWORD=CHANGE_ME_ROOT_PASSWORD -e MYSQL_DATABASE=FiberFix \
...
for i in {1..30}; do
    if docker exec add-dbms mysql -u root -pCHANGE_ME_ROOT_PASSWORD -e "SELECT 1" &>/dev/null; then
        break
    fi
    echo -n "."
    sleep 1
...
docker exec -i add-dbms mysql -u root -pCHANGE_ME_ROOT_PASSWORD FiberFix < "$PROJECT_ROOT/docker_listo/data/database.sql" 2>/dev/null
docker exec -i add-dbms mysql -u root -pCHANGE_ME_ROOT_PASSWORD FiberFix < "$PROJECT_ROOT/docker_listo/data/inserts.sql" 2>/dev/null
...
SERVER-PORT:5000
BBDD-USER:root
BBDD-PASS:CHANGE_ME_ROOT_PASSWORD
BBDD-HOST:localhost
BBDD-PORT:33006
EOF
